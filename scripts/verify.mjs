...
check('coque Tauri : permissions et cohérence', () => {
  ...
  // Identifiants vérifiés contre les dépôts officiels le 2026-09-18 :
  //   tauri-apps/tauri          → crates/tauri/permissions/window/autogenerated/reference.md
  //                                et crates/tauri/build.rs pour core:default
  //   tauri-apps/plugins-workspace (v2) → plugins/sql/permissions/
  const KNOWN = new Set([
    'core:default',
  ...
  const unknown = cap.permissions.filter((p) => !KNOWN.has(p));
  assert(
    unknown.length === 0,
    `identifiants de permission inconnus (le build échouerait) : ${unknown.join(', ')}`
  );

  // sql:default n'inclut PAS allow-execute : sans lui, aucune écriture.
  assert(
    cap.permissions.includes('sql:allow-execute'),
    'sql:allow-execute est indispensable : sql:default ne couvre que la lecture'
  );

  // La base préchargée doit être celle pour laquelle une migration existe,
  // sinon les migrations ne s'appliquent jamais au démarrage.
  const preload = (conf.plugins && conf.plugins.sql && conf.plugins.sql.preload) || [];
  const migrated = [...lib.matchAll(/add_migrations\("([^"]+)"/g)].map((m) => m[1]);
  for (const db of preload) {
    assert(migrated.includes(db), `« ${db} » préchargée mais sans migration enregistrée`);
  }

  // get_webview_window() vient du trait Manager : sans l'import, ça ne compile pas.
  assert(
    /use tauri::Manager;/.test(lib),
    'lib.rs doit importer le trait tauri::Manager (get_webview_window en dépend)'
  );

  const missing = (conf.bundle.icon || []).filter((p) => !fs.existsSync(path.join(TAURI, p)));
  assert(missing.length === 0, `icônes déclarées mais absentes : ${missing.join(', ')}`);
